...
const ShadowSystem = () => {
  ...
  const renderShadowGrid = (shadows: any[], title: string) => (
    <div className="bg-white dark:bg-black rounded-xl p-6 shadow-elevation-2 border border-neutral-200 dark:border-neutral-800">
      <h3 className="text-lg font-semibold text-neutral-900 dark:text-white mb-6">{title}</h3>
      
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {shadows.map((shadow) => (
          <div key={shadow.class} className="group">
            <div className="bg-gradient-to-br from-neutral-50 to-white dark:from-neutral-900 dark:to-neutral-800 p-8 rounded-xl border border-neutral-200 dark:border-neutral-700 mb-4 flex items-center justify-center">
              <div className={`w-20 h-20 bg-white dark:bg-neutral-800 rounded-xl ${shadow.class} flex items-center justify-center border border-neutral-100 dark:border-neutral-700`}>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <h4 className="font-medium text-neutral-800 dark:text-white">{shadow.name}</h4>
                <button
                  onClick={() => copyToClipboard(shadow.class, shadow.name)}
                  className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all duration-200"
                  title="Copiar classe"
                >
              ...
              <code className="text-xs bg-neutral-100 dark:bg-neutral-800 px-2 py-1 rounded font-mono text-neutral-700 dark:text-neutral-300 block">
                {shadow.class}
              </code>
  ...
  return (
    <div className="space-y-8 max-w-7xl">
      ...
      {/* Interactive Demo */}
      <div className="bg-white dark:bg-black rounded-xl p-6 shadow-elevation-2 border border-neutral-200 dark:border-neutral-800">
        <h3 className="text-lg font-semibold text-neutral-900 dark:text-white mb-6">Demonstração Interativa</h3>
        
        <div className="bg-gradient-to-br from-neutral-50 to-white dark:from-neutral-900 dark:to-neutral-800 p-12 rounded-xl border border-neutral-200 dark:border-neutral-700">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
            {shadowLevels.map((shadow, index) => (
              <div key={shadow.class} className="text-center">
                <div 
                  className={`w-16 h-16 bg-white dark:bg-neutral-800 rounded-xl ${shadow.class} mx-auto mb-3 border border-neutral-100 dark:border-neutral-700 hover:scale-110 transition-transform duration-200 cursor-pointer flex items-center justify-center`}
                  onClick={() => copyToClipboard(shadow.class, shadow.name)}
                >
                  <span className="text-sm font-semibold text-neutral-600 dark:text-neutral-300">{index + 1}</span>
                </div>
                <p className="text-xs text-neutral-600 dark:text-neutral-300">{shadow.name}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-gradient-to-br from-brand-50 to-white dark:from-neutral-900 dark:to-black rounded-xl p-6 border border-brand-200 dark:border-neutral-800">
        ...
        <div className="grid md:grid-cols-2 gap-6 text-sm">
          <div>
            <h4 className="font-medium text-neutral-800 dark:text-white mb-3">Hierarquia</h4>
